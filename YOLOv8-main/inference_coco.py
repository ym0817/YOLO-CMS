...
test_img = '/mnt/V8/FiterCoco/Val/images'
files = os.listdir(test_img)

result_json = 'voc_result.json'
    ...
    for i, det in enumerate(preds):  # detections per image
        ...
        if det is not None and len(det):
            ...
            for *xyxy, conf, cls_ in det:   # x1,y1,x2,y2
                ...
                temp = [os.path.splitext(file)[0], prob, int(xyxy[0]), int(xyxy[1]), int(xyxy[2]), int(xyxy[3])]
                if int(cls_) ==  0:
                    _list0.append(temp)
                if int(cls_) ==  1:
                    _list1.append(temp)
...
with open(result_json, 'w') as f_dump:
